...
(function () {
  var consent = "I agree to receive wedding reminder and livestream-alert texts from Joshua and Lucia (up to 4 messages: welcome, 7 days before, 1 day before, and the day of). Message and data rates may apply. Reply STOP to unsubscribe.";
  must(index, consent, "reminder form has on-page SMS consent");
  must(rsvpHtml, consent, "RSVP form has on-page SMS consent");
  must(index, 'type="checkbox" id="remSmsConsent" name="smsConsent" value="Yes"', "reminder SMS consent is a dedicated checkbox");
  must(rsvpHtml, 'type="checkbox" id="fSmsConsent" name="smsConsent" value="Yes"', "RSVP SMS consent is a dedicated checkbox");
  mustNot(index, /id="remSmsConsent"[^>]*\bchecked\b/, "reminder SMS checkbox must start unchecked");
  mustNot(rsvpHtml, /id="fSmsConsent"[^>]*\bchecked\b/, "RSVP SMS checkbox must start unchecked");
  mustNot(index, /By sharing your number, you agree/, "do not treat sharing a number as implied SMS consent");
  mustNot(rsvpHtml, /By sharing your number, you agree/, "do not treat sharing a number as implied SMS consent on RSVP");
  must(index, "https://jkgbafa.github.io/save-the-date/privacy.html", "reminder consent links to live privacy URL");
  must(index, "https://jkgbafa.github.io/save-the-date/terms.html", "reminder consent links to live terms URL");
  must(rsvpHtml, "https://jkgbafa.github.io/save-the-date/privacy.html", "RSVP consent links to live privacy URL");
  must(rsvpHtml, "https://jkgbafa.github.io/save-the-date/terms.html", "RSVP consent links to live terms URL");
  var remStart = index.indexOf('id="reminders"');
  var remEnd = index.indexOf('id="messages"');
  var remBlock = remStart >= 0 && remEnd > remStart ? index.slice(remStart, remEnd) : "";
  mustNot(remBlock, /gift|Venmo|Zelle|Mobile Money|honeymoon|donate|\bpay\b/i, "reminder consent must not mention gifts or money");
  var phoneStart = rsvpHtml.indexOf('id="fPhone"');
  var phoneEnd = rsvpHtml.indexOf('id="guestsField"');
  var phoneBlock = phoneStart >= 0 && phoneEnd > phoneStart ? rsvpHtml.slice(phoneStart, phoneEnd) : "";
  mustNot(phoneBlock, /gift|Venmo|Zelle|Mobile Money|honeymoon|donate|\bpay\b/i, "RSVP phone consent must not mention gifts or money");
  var remConsentStart = remBlock.indexOf('id="remSmsConsent"');
  var remConsent = remConsentStart >= 0 ? remBlock.slice(remConsentStart, remConsentStart + 900) : "";
  must(remConsent, "Privacy", "reminder SMS checkbox label links Privacy");
  must(remConsent, "Terms", "reminder SMS checkbox label links Terms");
})();
must(fs.readFileSync("js/main.js", "utf8"), 'smsConsent: smsOk ? "Yes" : "No"', "reminder submit sends SMS consent only if the box is ticked");
must(fs.readFileSync("js/rsvp.js", "utf8"), 'data.set("smsConsent", smsOk ? "Yes" : "No")', "RSVP submit sends SMS consent only if the box is ticked");
mustNot(fs.readFileSync("js/main.js", "utf8"), /remSmsConsent"\)\.checked\s*=\s*true/, "JS must not pre-check the reminder SMS box");
mustNot(fs.readFileSync("js/rsvp.js", "utf8"), /fSmsConsent"\)\.checked\s*=\s*true/, "JS must not pre-check the RSVP SMS box");
must(code, "function smsConsented_", "backend treats SMS opt-in only from the checkbox");
must(code, "if (smsOk) maybeSendReminderSms_", "welcome SMS is sent only when the SMS box is checked");
...
if (!fs.existsSync("privacy.html") || !fs.existsSync("terms.html")) {
  ...
  must(privacy, "up to 4 messages", "privacy states SMS frequency");
  must(privacy, "Message and data rates may apply", "privacy states message rates");
  must(privacy, "welcome, 7 days before, 1 day before, and the day of", "privacy lists the four reminder texts");
